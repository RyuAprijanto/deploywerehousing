...
<div class="container flex h-screen w-screen justify-center">
    <div id="editproduct" class="justify-center items-center w-screen h-full m-4">
                    ...
                    <form class="space-y-6" action="{{ route('updateProduct', ['id' => $product->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6">
                            <div class="z-0 w-full my-3 group">
                                <label for="type" required class="text-sm font-medium leading-6 text-gray-900">Kategori Produk</label>
                                <select id="type" name="type_id" class="form-group block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                    ...
                                    @foreach ($types as $id => $name)
                                        <option value="{{ $id }}" {{ $id == old('type_id', $product->type_id) ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            
                            <div class="z-0 w-full my-3 group">
                                <input type="checkbox" id="enable_new_type" name="enable_new_type" class="form-group mr-2" {{ old('enable_new_type', $product->type_id) == 'new' ? 'checked' : '' }}>
                            ...
                            <div class="z-0 w-full my-3 group">
                                <label for="stock" class="text-sm font-medium leading-6 text-gray-900">Jumlah Produk</label>
                            ...
                            <div class="z-0 w-full my-3 group">
                                <label for="buy_price" class="text-sm font-medium leading-6 text-gray-900">Harga Beli</label>
                                <input id="buy_price" name="buy_price" value="{{ old('buy_price', $product->buy_price) }}" type="number" required class="form-group block min-w-9 rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                            </div>

                            <div class="z-0 w-full my-3 group">
                                <label for="sell_price" class="text-sm font-medium leading-6 text-gray-900">Harga Jual</label>
                                <input id="sell_price" name="sell_price" value="{{ old('sell_price', $product->sell_price) }}" type="number" required class="form-group block min-w-9 rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                            </div>

                            <div class="z-0 w-full my-3 group">
                                <input type="checkbox" id="has_expiry_date" name="has_expiry_date" class="form-group mr-2" {{ old('has_expiry_date', !is_null($product->expired_date)) ? 'checked' : '' }}>
                                <label for="has_expiry_date" class="text-sm font-medium leading-6 text-gray-900">Tanggal Kedaluarsa</label>
                            </div>

                            <div id="expiry_date_container" class="z-0 w-full my-3 group {{ is_null($product->expired_date) ? 'hidden' : '' }}">
                                <label for="expired_date" class="text-sm font-medium leading-6 text-gray-900">Tanggal Kedaluwarsa</label>
                                <input id="expired_date" name="expired_date" type="date" value="{{ old('expired_date', $product->expired_date) }}" class="form-group block rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                            </div>
                        </div>
